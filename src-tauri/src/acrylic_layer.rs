...
pub fn setup_windows_appbar(window: &WebviewWindow, monitor: &tauri::Monitor, dock_height: u32) {
    ...
    unsafe {
        ...
        appbar_data.rc.bottom = appbar_data.rc.top + physical_dock_height;
...
pub async fn create_widget_area(app: AppHandle, monitor_id: String) -> Result<String, String> {
    ...
    let mut target_x = monitor_info.x;
    let mut target_y = monitor_info.y;
    let mut target_w = monitor_info.width;
    let mut target_h = monitor_info.height;

    #[cfg(target_os = "windows")]
    unsafe {
        let pt = windows::Win32::Foundation::POINT { x: target_x as i32, y: target_y as i32 };
        let hmonitor = MonitorFromPoint(pt, MONITOR_DEFAULTTONEAREST);
        if !hmonitor.is_invalid() {
            let mut mi = MONITORINFO { cbSize: std::mem::size_of::<MONITORINFO>() as u32, ..Default::default() };
            if GetMonitorInfoW(hmonitor, &mut mi).into() {
                target_x = mi.rcWork.left as f64;
                target_y = mi.rcWork.top as f64;
                target_w = (mi.rcWork.right - mi.rcWork.left) as f64;
                target_h = (mi.rcWork.bottom - mi.rcWork.top) as f64;
            }
        }
    }
    ...
    let _ = window.set_position(tauri::PhysicalPosition::new(target_x as i32, target_y as i32));
