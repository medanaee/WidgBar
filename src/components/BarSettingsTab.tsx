...
import { GripVertical, Plus, Trash2, LayoutGrid, Settings, Link2, Download, Upload, AlertTriangle } from 'lucide-react';
import { exportBarPreset, importBarPreset, openJsonFile, BarPresetExport } from '../lib/presetExportImport';
...
export default function BarSettingsTab({
  ...
  const [importPending, setImportPending] = useState<{ jsonStr: string; preset: BarPresetExport } | null>(null);
              ...
              <SettingCardNoLayout className="flex flex-col gap-3">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                    {t("exportPresetDesc") || "Export or import Bar layout and widget configs as JSON."}
                  </p>
                </div>
                <div className="flex items-center gap-2 w-full">
                  <button
                    type="button"
                    onClick={() => exportBarPreset(selectedMonitorId)}
                    className="flex flex-1 items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 rounded-lg transition-colors border border-zinc-500/20"
                  >
                ...
                <div className="flex flex-col gap-4" dir={language === 'fa' ? 'rtl' : 'ltr'}>
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-zinc-500/10">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const res = importBarPreset(selectedMonitorId, importPending.jsonStr);
