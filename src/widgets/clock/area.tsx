...
export default function ClockArea({ widgetId }: { widgetId: string }) {
    ...
    if (clockType === 'analog_macos') {
        return (
            <div className="flex items-center justify-center w-full h-full p-3 select-none pointer-events-none">
                <svg viewBox="0 0 200 200" className="w-full h-full max-w-full max-h-full drop-shadow-[0_4px_12px_rgba(0,0,0,0.12)]">
                    {/* 12 Hour Ticks - Completely transparent dial */}
                    {Array.from({ length: 12 }).map((_, h) => {
                        ...
                        const strokeColor = isMajor 
                            ...
                            : "stroke-zinc-500/50 dark:stroke-zinc-400/40";
                        
                        const x1 = 100 + (92 - length) * Math.sin(angle);
                        const y1 = 100 - (92 - length) * Math.cos(angle);
                    ...
                    {/* Hour Hand (macOS Rounded Rect Needle) */}
                    <line 
                        x1="100" y1="100" 
                        x2={100 + 44 * Math.sin(hourAngle * (Math.PI / 180))} 
                        y2={100 - 44 * Math.cos(hourAngle * (Math.PI / 180))} 
                    ...
                    {/* Minute Hand (macOS Rounded Rect Needle) */}
                    <line 
                        x1="100" y1="100" 
                    ...
                    {showSeconds && (
                        <>
                            ...
                            <line 
                                x1="100" y1="100" 
                                x2={100 - 18 * Math.sin(secondAngle * (Math.PI / 180))} 
                                y2={100 + 18 * Math.cos(secondAngle * (Math.PI / 180))} 
                                className="stroke-orange-500 dark:stroke-orange-400" 
                                strokeWidth="1" 
                                strokeLinecap="round" 
                            />
